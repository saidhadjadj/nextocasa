...
function Achat() {
  ...
  useEffect(() => {
    fetch('/properties.json')
      .then(res => res.json())
      .then(data => {
        const achatProperties = data.filter(p => p.operation === 'Vente');
        setProperties(achatProperties);
        setFilteredProperties(achatProperties);
        setLoading(false);
      })
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-2xl text-blue-600 animate-pulse">⏳ Chargement des biens à vendre...</div>
      </div>
    );
  }
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 space-y-6">
              <div>
                ...
                <div className="flex justify-between mt-2 text-sm text-gray-600">
                  <span>0 €</span>
                  <span>{priceRange.max.toLocaleString('fr-FR')} €</span>
                </div>
              </div>
