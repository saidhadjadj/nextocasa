  ...
  return (
    <div className="prose-nextocasa">
      {paragraphs.map((line, i) => {
        if (line.startsWith("## ")) {
          return (
            <h2 key={i} className="font-serif text-2xl font-light text-stone-800 mt-10 mb-4 leading-snug">
              {line.replace("## ", "")}
            </h2>
          );
        }
        if (line.startsWith("> **Chiffre clé**") || line.startsWith("> **")) {
          return (
            <div key={i} className="my-6 border-l-2 border-[#ffb800] pl-5 py-2 bg-[#ffb800]/5 rounded-r-xl">
              <p
                className="text-sm font-light text-stone-600 leading-relaxed italic"
                dangerouslySetInnerHTML={{
                  __html: line.replace(/^> /, "").replace(/\*\*(.*?)\*\*/g, '<strong class="font-semibold text-stone-800">$1</strong>'),
                }}
              />
            </div>
          );
        }
        if (line.startsWith("Pour aller plus loin")) return null;
        if (line.includes("**")) {
          return (
            <p
              key={i}
              ...
              dangerouslySetInnerHTML={{
                __html: line.replace(/\*\*(.*?)\*\*/g, '<strong class="font-semibold text-stone-800">$1</strong>'),
              }}
            />
          );
        }
        return (
          <p key={i} className="text-stone-600 text-base font-light leading-relaxed mb-4">
            {line}
          </p>
        );
      })}
              ...
              <div className="space-y-2">
                {article.interlinks.map((iSlug) => {
                  ...
                  return (
                    ...
                    >
                      <svg className="w-3.5 h-3.5 shrink-0 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <div className="mt-10 rounded-2xl bg-[#0022d2] px-8 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                <p className="text-[#ffb800]/70 text-xs tracking-[0.18em] uppercase font-light">Votre projet</p>
              </div>
