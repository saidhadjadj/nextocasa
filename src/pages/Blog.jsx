...
import { CATEGORIES, BLOG_BENEFITS } from "../data/blogContent";
import { useSanityArticles } from "../hooks/useSanityArticles";

function ArticleCard({ article }) {
  return (
    <Link
      to={`/observatoire/${article.slug}`}
    ...
    >
      <div className="relative h-48 overflow-hidden bg-stone-100">
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          ...
          {CATEGORIES.find(c => c.key === article.category)?.icon} {article.tag}
          </span>
          {article.type === "pilier" && (
...
export default function Blog() {
  ...
  const { articles, loading } = useSanityArticles();
  ...
  if (loading) return (
    <div className="bg-[#f7f5f1] min-h-screen flex items-center justify-center">
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="L'Observatoire NextoCasa — analyses du marché parisien, conseils acheteurs et guides investisseurs par nos experts." />
      </Helmet>

      <div className="bg-[#f7f5f1] min-h-screen">
        <div className="relative h-64 md:h-80 overflow-hidden">
          <img
            src="https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=1200&q=70&fm=webp&auto=format"
            alt=""
            aria-hidden="true"
          ...
          <div className="relative h-full flex flex-col items-center justify-center text-center px-6">
            ...
            <h1 className="font-serif text-4xl md:text-5xl font-light text-white mb-3">
              L'<em className="not-italic italic text-[#ffb800]">Observatoire</em> NextoCasa
            </h1>
            <p className="text-white/60 text-sm font-light max-w-lg">
              Analyses de marché, décryptages et conseils d'experts. Tout ce qu'il faut savoir pour décider avec clarté.
            </p>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          {featuredArticle && activeCategory === "all" && search === "" && (
            <div className="mb-14">
              <div className="flex items-center gap-3 mb-6">
                <div className="h-px w-6 bg-[#ffb800]" />
                <p className="text-xs tracking-[0.2em] uppercase text-stone-400 font-light">À la une</p>
              </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between mb-8">
            ...
            <div className="relative shrink-0">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-stone-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-20">
              <p className="font-serif text-3xl font-light text-stone-300 mb-3">Aucun article</p>
              <button
          ...
          <div className="mt-20 rounded-3xl bg-[#0022d2] px-8 py-14 md:px-16">
            <div className="flex items-center justify-center gap-3 mb-3">
              ...
              <p className="text-[#ffb800]/70 text-xs tracking-[0.2em] uppercase font-light">Pourquoi ce blog</p>
              <div className="h-px w-6 bg-[#ffb800]/50" />
            </div>
            <h2 className="font-serif text-2xl md:text-3xl font-light text-white text-center mb-12">
              L'Observatoire au service de <em className="not-italic italic text-[#ffb800]">votre projet</em>
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {BLOG_BENEFITS.map((b) => (
                <div key={b.title} className="text-center">
                  <span className="text-3xl mb-4 block" aria-hidden="true">{b.icon}</span>
                  <h3 className="font-serif text-lg font-light text-[#ffb800] mb-2">{b.title}</h3>
